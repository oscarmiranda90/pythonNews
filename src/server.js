...
// Internal FastAPI/uvicorn backend
const INTERNAL_API_PORT = Number.parseInt(process.env.INTERNAL_API_PORT ?? "8081", 10);
const INTERNAL_API_HOST = process.env.INTERNAL_API_HOST ?? "127.0.0.1";
const API_TARGET = `http://${INTERNAL_API_HOST}:${INTERNAL_API_PORT}`;
const PYTHON_VENV = process.env.PYTHON_VENV ?? "/app/venv";
const PYTHONPATH = process.env.PYTHONPATH ?? "/app/python";

let apiProc = null;

function startApi() {
  const uvicorn = `${PYTHON_VENV}/bin/uvicorn`;
  apiProc = childProcess.spawn(
    uvicorn,
    ["backend.main:app", "--host", INTERNAL_API_HOST, "--port", String(INTERNAL_API_PORT)],
    {
      stdio: "inherit",
      env: { ...process.env, PYTHONPATH, OPENCLAW_GATEWAY_URL: GATEWAY_TARGET },
    }
  );
  ...
  console.log(`[api] uvicorn started → ${API_TARGET}`);
}
...
// Proxy for FastAPI backend — handles /api/* and /health
const apiProxy = httpProxy.createProxyServer({ target: API_TARGET, xfwd: true });
